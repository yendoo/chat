...
public class ChatDAO {
	...
	public ArrayList<ChatDTO> getChatListByRecent(String fromID, String toID,int number){
		...
		ResultSet rs = null;
		String SQL = "SELECT * FROM CHAT WHERE ((fromID = ? AND toID = ?) OR "
				+ "(fromID = ? AND toID=?)) "
				+ "AND chatID > (SELECT MAX(chatID) - ? "
				+ "FROM CHAT WHERE (fromID = ? AND toID = ?)"
				+ " OR (fromID = ? AND toID = ? )) "
				+ "ORDER BY chatTime";
		try {
			conn = dataSource.getConnection();
			...
			while(rs.next()) {
				...
				chat.setChatTime(rs.getString("chatTime").substring(0,11) + " " + timeType + chatTime + ":".substring(14, 16));
				chatList.add(chat);
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			for (int i = 0; i < chatList.size(); i++) {
				...
				for(int j = 0; j < chatList.size(); j++) {
					...
					if(x.getFromID().equals(y.getToID()) && x.getToID().equals(y.getFromID())){ //fromID 가 다른 chatID의 toID와 같고 다른 메시지의 toID가 다른 fromID와 같다며
						if(x.getChatID() < y.getChatID()) { //더 최신인 chatID만 남김
							chatList.remove(x); 
							i--;
							break;
						} else {
